class Api {
  ...
  static async getTwitterAdress() {
    try {
      const data = await Api.take();
      if (data && data.twitter) {
        console.log("✓ Api.getTwitterAdress \t\t\t—▶ Successfully! returned twitter adress : ", data.twitter);
        return data.twitter;
      } else {
        console.error("✕ Api.getTwitterAdress \t\t\t—▶ No twitter found in API response.");
        return null;
      }
    } catch (err) {
      console.error("✕ Api.getTwitterAdress \t\t\t—▶ Ups! Something went wrong.\n", err);
      return null;
    }
  }
